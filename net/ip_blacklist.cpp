...
bool ip_blacklist_get(bool download_diff_restart)
{
    ...
    char *kiwisdr_com = DNS_lookup_result("ip_blacklist_get", "rx-888.com", &net.ips_kiwisdr_com);
    #define BLACKLIST_FILE "ip_blacklist/ip_blacklist3.cjson"

    asprintf(&cmd_p, "curl -L -s -f --ipv4 --connect-timeout 5 \"%s/%s\" 2>&1", kiwisdr_com, BLACKLIST_FILE);
