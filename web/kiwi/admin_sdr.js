...
function config_html()
{
   ...
   var s6 =
		'<hr>' +
		w3_div('w3-margin-bottom w3-text-teal ui-admin-config-options',
			w3_div('w3-restart ui-admin-config-option',
            w3_div('ui-admin-config-option-control',
               w3_switch_label('w3-center', 'External Reference clock?', 'Yes', 'No',
                  'ext_ADC_clk', cfg.ext_ADC_clk, 'config_ext_clk_sel_cb')
            ) +
				w3_text('w3-text-black ui-admin-config-option-desc',
               'Set when external 10.000 MHz clock is connected to the external clock connector or pad.')
			) +
			w3_div('w3-restart ui-admin-config-option',
            w3_div('ui-admin-config-option-control',
               w3_select('', 'GPS correction of ADC clock', '', 'cfg.ADC_clk2_corr',
                  cfg.ADC_clk2_corr, admin_sdr.ADC_clk2_corr_s, 'admin_select_cb')
            ) +
				w3_text('w3-text-black ui-admin-config-option-desc',
         ...
         w3_div('w3-restart ui-admin-config-option',
            w3_div('ui-admin-config-option-control',
               w3_input('', 'External output clock frequency (enter in Hz)',
                  'ext_ADC_freq', cfg.ext_ADC_freq, 'config_ext_freq_cb')
            ) +
            w3_text('w3-text-black ui-admin-config-option-desc',
               'Set the output clock frequency in Hz. Use 0 to disable it. ' +
               'The output is disabled while External Reference clock is enabled.')
         )
		);
