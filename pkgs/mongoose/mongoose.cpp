...
#define NSF_UDP                     (1 << 8)
#define NSF_HTTP_TO_HTTPS_REDIRECT  (1 << 9)
...
struct ns_connection *ns_bind(struct ns_mgr *srv, const char *str,
                              ns_callback_t callback, void *user_data, bool *bad_cert) {
  union socket_address sa;
  ...
  if ((sock = ns_open_listening_socket(&sa, proto)) == INVALID_SOCKET) {
    ...
    if (use_ssl) {
      ...
      if (ns_use_cert(nc->ssl_ctx, cert) != 0 ||
        ...
        //printf("ns_bind: SSL cert validation failure\n");
        ns_close_conn(nc);
        nc = NULL;
        *bad_cert = true;
      }
...
static struct ns_connection *accept_conn(struct ns_connection *ls) {
  ...
  if ((sock = accept(ls->sock, &sa.sa, &len)) == INVALID_SOCKET) {
    ...
    DBG(("SSL error"));
    //printf("accept_conn: SSL cert validation failure\n");
    ns_close_conn(c);
    c = NULL;
#endif
  } else {
  ...
  if (buf != NULL && len > 0) {
    ...
    if (flags & 2) {
      ...
      if (flags & 1) {
        ...
        snprintf(buf, len, "[%s]:%d", buf2, (int) ntohs(sa.sin.sin_port));
      } else
#endif
      {
        snprintf(buf, len, "%s%s%d", flags & 1 ? buf2 : "",
...
xreal_printf("ns_read_from_socket: TOP flags=0x%x\n", conn->flags);
  if (conn->flags & NSF_CONNECTING) {
    ...
    DBG(("%p ok=%d", conn, ok));
    if (ok != 0) {
xreal_printf("ns_read_from_socket: !OK NS_CONNECT NSF_CLOSE_IMMEDIATELY\n");
      conn->flags |= NSF_CLOSE_IMMEDIATELY;
    }
    ns_call(conn, NS_CONNECT, &ok);
xreal_printf("ns_read_from_socket: NS_CONNECT OK\n");
    return;
  }

#ifdef NS_ENABLE_SSL
  if (conn->ssl != NULL && !(conn->flags & NSF_HTTP_TO_HTTPS_REDIRECT)) {
    if (conn->flags & NSF_SSL_HANDSHAKE_DONE) {
      ...
      if (res == 1) {
        ...
        //conn->flags |= NSF_CLOSE_IMMEDIATELY;
        conn->flags |= NSF_HTTP_TO_HTTPS_REDIRECT;
xreal_printf("ns_read_from_socket: NSF_HTTP_TO_HTTPS_REDIRECT res=%d ssl_err=%d\n", res, ssl_err);
      }
      return;
    }
  } else
#endif
  {
xreal_printf("ns_read_from_socket: normal read loop..\n");
    while ((n = (int) recv(conn->sock, buf, sizeof(buf), 0)) > 0) {
      DBG(("%p %d <- %d bytes (PLAIN)", conn, conn->flags, n));
      iobuf_append(&conn->recv_iobuf, buf, n);
xreal_printf("ns_read_from_socket: ns_call PRE NS_RECV n=%d <%s>\n", n, buf);
      ns_call(conn, NS_RECV, &n);
xreal_printf("ns_read_from_socket: ns_call POST NS_RECV n=%d\n", n);
    }
xreal_printf("ns_read_from_socket: WHILE DONE n=%d\n", n);
  }

  if (ns_is_error(n)) {
    conn->flags |= NSF_CLOSE_IMMEDIATELY;
xreal_printf("ns_read_from_socket: ns_is_error n=%d errno=%d\n", n, errno);
  }
}
...
#ifdef NS_ENABLE_SSL
  // If NSF_HTTP_TO_HTTPS_REDIRECT is in effect need to bypass SSL on output
  // so the redirect headers are sent as unencrypted HTTP.
  if (conn->ssl != NULL && !(conn->flags & NSF_HTTP_TO_HTTPS_REDIRECT)) {
...
struct ns_connection *ns_connect(struct ns_mgr *mgr, const char *address,
  ...
  if (use_ssl) {
    if ((nc->ssl_ctx = SSL_CTX_new(SSLv23_client_method())) == NULL ||
      ...
      //printf("ns_connect: SSL: cert validation error\n");
      ns_close_conn(nc);
      return NULL;
    } else {
...
xreal_printf("parse_http_message len=%d\n", len);
  if (len < 1) return ~0;

  buf[len - 1] = '\0';
xreal_printf("parse_http_message buf=<%s>\n", buf);
...
xreal_printf("parse_http_message-0 request_method=<%s> valid=%s uri=<%s> http_version=<%s>\n",
    ri->request_method, is_valid_http_method(ri->request_method)? "T":"F", ri->uri, ri->http_version);
...
xreal_printf("parse_http_message len = ~0\n");
    len = ~0;
  } else {
    if (is_request) {
      ri->http_version += 5;
...
xreal_printf("parse_http_message len=%d request_method=<%s> uri=<%s> http_version=<%s> status_code=%d\n",
    len, ri->request_method, ri->uri, ri->http_version, ri->status_code);
...
static void open_file_endpoint(struct connection *conn, const char *path,
  ...
  mg_send_standard_headers(mc, path, st, msg, range, extra_headers);
...
void mg_send_standard_headers(struct mg_connection *mc, const char *path, file_stat_t *st,
  ...
  char date[64], lm[64], etag[64], headers[1000];
  time_t curtime = time(NULL);
  struct vec mime_vec;
  int n;
  if (range == NULL) range = (char *) "";
  bool term_cr_nl = (range[0] != '\0' || extra_headers != NULL || MONGOOSE_USE_EXTRA_HTTP_HEADERS[0] != '\0');
...
xreal_printf("try_parse-0 request_len=%d\n", conn->request_len);
  if (conn->request_len == 0 &&
    ...
    // become invalid.
xreal_printf("try_parse-1 request_len=%d\n", conn->request_len);
#ifdef NS_ENABLE_SSL
    // put back first 5 bytes ("GET /") that SSL setup processing takes away from i/o buffer
    if (conn->ns_conn->flags & NSF_HTTP_TO_HTTPS_REDIRECT) {
      conn->request = (char *) NS_MALLOC(conn->request_len + 5);
      memcpy(conn->request, "GET /", 5);
      memcpy(conn->request + 5, io->buf, conn->request_len);
      iobuf_remove(io, conn->request_len);
      conn->request_len += 5;
xreal_printf("try_parse NSF_HTTP_TO_HTTPS_REDIRECT request_len=%d\n", conn->request_len);
    } else
#endif
    {
    ...
    conn->request_len = parse_http_message(conn->request, conn->request_len,
                                           &conn->mg_conn);
xreal_printf("try_parse-2 request_len=%d\n", conn->request_len);
    if (conn->request_len > 0) {
...
xreal_printf("try_parse-3 content_len=%d\n", conn->cl);
    }
  }
}
...
static void on_recv_data(struct connection *conn) {
  ...
  DBG(("%p %d %lu %d", conn, conn->request_len, (unsigned long)io->len,
       conn->ns_conn->flags));
xreal_printf("on_recv_data: rl=%d iol=%lu flags=0x%x\n", conn->request_len, (unsigned long)io->len,
   conn->ns_conn->flags);
  if (conn->request_len < 0 ||
      (conn->request_len > 0 && !is_valid_uri(conn->mg_conn.uri))) {
    send_http_error(conn, 400, NULL);
#ifdef NS_ENABLE_SSL
  } else if (conn->ns_conn->flags & NSF_HTTP_TO_HTTPS_REDIRECT) {
      // If HTTP header parse worked when NSF_HTTP_TO_HTTPS_REDIRECT is in effect it means
      // this is an attempted HTTP connection on a port configured for SSL and we should
      // respond with a redirect to HTTPS.
      char https_uri[256];
      struct mg_connection *mc = &conn->mg_conn;
      mg_snprintf(https_uri, sizeof(https_uri), "https://%s%s%s", mg_get_header(mc, "Host"),
        (mc->uri[0] != '/')? "/" : "", mc->uri);
      printf("webserver: HTTP => HTTPS/SSL redirect <%s>\n", https_uri);
      mg_send_status(mc, 307);
      mg_send_header(mc, "Location", https_uri);
...
static void close_local_endpoint(struct connection *conn) {
  ...
  c->callback_param = NULL;     // KiwiSDR: leave connection_param alone
  ...
  if (ind == LISTENING_PORT) {
    ...
    while ((value = next_option(value, &vec, NULL)) != NULL) {
      ...
      struct ns_connection *c = ns_bind(&server->ns_mgr, vec.ptr,
                                        mg_ev_handler, NULL, &bad_cert);

      // if it's only a bad cert keep going in case other non-SSL ports are specified
      if (bad_cert) {
        lprintf("webserver: SSL cert validation failure, port %.*s\n", vec.len, vec.ptr);
        lprintf("webserver: => check content of the cert.pem cert file\n");
        continue;
      } else
      if (c == NULL) {
        error_msg = "Cannot bind to port";
        lprintf("webserver: cannot bind to port %.*s\n", vec.len, vec.ptr);
        break;
      } else {
