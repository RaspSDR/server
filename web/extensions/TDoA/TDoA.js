...
var tdoa = {
   ...
   submit_status: [
      ...
      { n:1,   f:1,  m:'Octave' },
      { n:2,   f:0,  m:'a sampling error' },
      { n:3,   f:0,  m:'no GPS timestamps at all' },
      { n:4,   f:0,  m:'no recent GPS timestamps' },
      { n:5,   f:1,  m:'pair map' },
      { n:6,   f:1,  m:'combined map' },
      { n:7,   f:1,  m:'dt plot' },
      { n:8,   f:0,  m:'out of memory: use fewer Devices or check signal quality' },
...
function tdoa_rebuild_hosts(opts)
{
   ...
   if (tdoa.cur_host_markers) {
      tdoa.hosts.forEach(function(h) {
         h.mkr.unbindTooltip(); h.mkr.remove(); h.mkr.off();
      });
   ...
   for (var i = 0; i < tdoa.hosts.length; i++) {
      ...
      if (h.mkr) {
         ...
         if (h.selected) {
            ...
            tdoa_style_marker(h.mkr, h.idx, id, 'host', tdoa.kiwi_map);
         } else {
            if (show_hosts) {
               tdoa_style_marker(h.mkr, h.idx, id, 'host');
      ...
      w3_inline_percent('w3-container',
         ...
         w3_div('',
            ...
            w3_div('w3-margin-T-8 w3-text-black',
               ...
               'admin page Public tab or the grid square computed from your device lat/lon.'
            )
         ), 40,
