...
static void dx_flag(dx_t* dxp, const char* flag) {
    ...
    if (strcmp(flag, "WL") == 0)
        dxp->flags |= DX_WL;
    else if (strcmp(flag, "SB") == 0)
        dxp->flags |= DX_SB;
    else if (strcmp(flag, "DG") == 0)
        dxp->flags |= DX_DG;
    else if (strcmp(flag, "NoN") == 0)
        dxp->flags |= DX_SE;
    else // deprecated, convert to "SE" on next file write
        if (strcmp(flag, "SE") == 0)
            dxp->flags |= DX_SE;
        else if (strcmp(flag, "XX") == 0)
            dxp->flags |= DX_XX;
        else if (strcmp(flag, "PB") == 0)
            ;
        else // deprecated, but here in case any json file still has it
            if (strcmp(flag, "MK") == 0)
                dxp->flags |= DX_MASKED;
            else
...
void update_masked_freqs(dx_t* _dx_list, int _dx_list_len) {
    ...
    kiwi_ifree(dx.masked_list, "dx masked");
    dx.masked_list = NULL;
...
static jsmntok_t* dx_reload_element(cfg_t* cfg, dx_db_t* dx_db, jsmntok_t* jt, jsmntok_t* end_tok, dx_t* dxp, int* error) {
    ...
    *error = E_UNEXPECTED;
    ...
    double f;
    if (cfg_float_tok(cfg, jt, &f) == false) {
        *error = E_FREQ;
        return NULL;
    }
    dxp->freq = f;
    ...
    const char* mode_s;
    if (cfg_string_tok(cfg, jt, &mode_s) == false) {
        *error = E_MODE;
        return NULL;
    }
    ...
    if (cfg_string_tok(cfg, jt, &s) == false) {
        *error = E_IDENT;
        return NULL;
    }
    ...
    if (jt != end_tok && JSMN_IS_OBJECT(jt)) {
        ...
        while (jt != end_tok && !JSMN_IS_ARRAY(jt)) {
            ...
            const char* id;
            if (cfg_string_tok(cfg, jt, &id) == false) {
                *error = E_OPT_ID;
                return NULL;
            }
            jt++;
            ...
            else if (cfg_string_tok(cfg, jt, &s) == true) {
                // printf("dx-json %s=<%s>\n", id, s);
                if (strcmp(id, "p") == 0) {
                    kiwi_str_unescape_quotes((char*)s);
...
#define NTOK 32
            ...
            if ((s = fgets(lbuf, LBUF, fp)) != NULL) {
                ...
                if (ntok_or_err < 0) {
                    if (strcmp(s, "]}") != 0) { // i.e. "]}" on last line
                        ...
                        if (ntok_or_err == JSMN_ERROR_INVAL)
                            err = "invalid character inside JSON string";
                        else if (ntok_or_err == JSMN_ERROR_PART)
    ...
    fclose(fp);
    kiwi_free("dx tokens", cfg->tokens);
    cfg->tokens = NULL;
...
static bool dx_download_file(const char* host, const char* src, const char* dst) {
    ...
    else if (kiwi_file_size(tmp) > 256) {
...
bool dx_community_get(bool download_diff_restart) {
    bool restart = false;
    char* rx888_downloads = DNS_lookup_result("dx_community_get", "downloads.rx-888.com", &net.ips_downloads);
    ...
    if (admcfg_bool("dx_comm_auto_download", NULL, CFG_OPTIONAL) || download_oneshot) {
        restart |= dx_download_file(rx888_downloads, "webconfig/dx_community_config.json", DIR_CFG "/dx_community_config.json");
        restart |= dx_download_file(rx888_downloads, "webconfig/dx_community.json", DIR_CFG "/dx_community.json");
    }
