    ...
    *is_file = false;
    ...
    if (!data) {
        ...
        if (cache_check ||
            ...
            else if ((fd = open(uri2, O_RDONLY)) < 0) {
                fail = true;
            }
            else {
...
//#define WEB_PRINTF_ON_FILE
    ...
    evWS(EC_EVENT, EV_WS, 0, "WEB_SERVER", evprintf("URL <%s> <%s> %s", o_uri, mc->query_string, (evt == MG_CACHE_INFO) ? "MG_CACHE_INFO" : "MG_REQUEST"));
    ...
    else if (strncmp(o_uri, "extensions/", 11) == 0) {
        uri = o_uri;
        has_prefix = TRUE;
        is_extension = TRUE;
    }
    else if (strncmp(o_uri, "pkgs/", 5) == 0) {
        uri = o_uri;
        has_prefix = TRUE;
    }
    else if (strncmp(o_uri, "pkgs_maps/", 10) == 0) {
        uri = o_uri;
        has_prefix = TRUE;
    }
    else if (strncmp(o_uri, "config/", 7) == 0) {
        asprintf(&uri, "%s/%s", DIR_CFG, &o_uri[7]);
        if (MTR) real_printf("-> uri-1 ASPR %p %d<%s>\n", uri, strlen(uri), uri);
        free_uri = TRUE;
        has_prefix = TRUE;
    }
    else if (strncmp(o_uri, "kiwi.config/", 12) == 0) {
        asprintf(&uri, "%s/%s", DIR_CFG, &o_uri[12]);
        if (MTR) real_printf("-> uri-2 ASPR %p %d<%s>\n", uri, strlen(uri), uri);
        free_uri = TRUE;
        has_prefix = TRUE;
    }
    else if (strncmp(o_uri, "kiwi.data/", 10) == 0) {
...
#define NQS 32
        ...
        for (i = 0; i < n; i++) {
            ...
            else if (strcmp(qs[i].str, "cache") == 0) {
                web_nocache = false;
                printf("WEB cache\n");
            }
            else if (sscanf(qs[i].str, "ctrace=%d", &ctrace) == 1) {
    ...
    if (evt == MG_CACHE_INFO) {
        ...
        else if (web_nocache || !webserver_caching) {
