            ...
            if (badp == BADP_OK || badp == BADP_ADMIN_CONN_ALREADY_OPEN) {
                ...
                if (conn->auth == false) {
                    ...
                    if (stream_snd_or_wf || stream_admin_or_mfg) {
                        send_msg(conn, SM_NO_DEBUG, "MSG version_maj=%d version_min=%d debian_ver=%d platform=%d ext_clk=%d",
                            version_maj, version_min, debian_ver, kiwi.platform, kiwi.ext_clk);
                    }
